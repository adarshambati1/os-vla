...
WRENCHDIR = "/workspace/vla_data/wrench"
TASKS = [
...
class LiberoForcewrench(tfds.core.GeneratorBasedBuilder):
    VERSION = tfds.core.Version("1.0.0")
    RELEASE_NOTES = {"1.0.0": "Heavy-contact LIBERO-90; action = pose(6) + commanded 6-DOF wrench + gripper; wrench in proprio."}
        ...
        return self.dataset_info_from_configs(
            features=tfds.features.FeaturesDict({
                "steps": tfds.features.Dataset({
                    ...
                    "action": tfds.features.Tensor(shape=(13,), dtype=np.float32),
    ...
    def _generate_examples(self) -> Iterator[Tuple[str, Any]]:
        for name in TASKS:
            ...
            wr = np.load(f"{WRENCHDIR}/{name}.npz")
            f = h5py.File(path, "r")
            data = f["data"]
            for dn in data.keys():
                obs = data[dn]["obs"]
                ...
                ee = np.concatenate([obs["ee_pos"][:], obs["ee_ori"][:], obs["gripper_states"][:]], axis=1)
                wrench = np.asarray(wr[dn], np.float32)
                state = np.concatenate([ee, wrench], axis=1).astype(np.float32)
                action = np.concatenate([acts[:, :6], wrench, acts[:, 6:7]], axis=1).astype(np.float32)
                n = len(acts)
                steps = []
                for t in range(n):
                    steps.append({
