...
class LiberoJoint(tfds.core.GeneratorBasedBuilder):
    VERSION = tfds.core.Version("1.0.0")
    RELEASE_NOTES = {"1.0.0": "Heavy-contact LIBERO-90, joint-delta actions (7 joints + gripper)."}
    ...
    def _generate_examples(self) -> Iterator[Tuple[str, Any]]:
        for name in TASKS:
            ...
            for dn in data.keys():
                ...
                osc = np.asarray(data[dn]["actions"][:], np.float32)
                joints = np.asarray(obs["joint_states"][:], np.float32)
                grip = obs["gripper_states"][:]
                state = np.concatenate([obs["ee_pos"][:], obs["ee_ori"][:], grip], axis=1).astype(np.float32)
                n = len(osc)
                jdelta = np.zeros((n, 7), np.float32)
                jdelta[:-1] = joints[1:] - joints[:-1]
                action = np.concatenate([jdelta, osc[:, -1:]], axis=1).astype(np.float32)
                steps = []
                for t in range(n):
                    steps.append({
                        "observation": {
