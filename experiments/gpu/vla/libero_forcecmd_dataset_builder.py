...
class LiberoForcecmd(tfds.core.GeneratorBasedBuilder):
    VERSION = tfds.core.Version("1.0.0")
    RELEASE_NOTES = {"1.0.0": "Heavy-contact LIBERO-90; action = pose(6) + commanded Fz + gripper; wrench in proprio."}
    ...
    def _generate_examples(self) -> Iterator[Tuple[str, Any]]:
        for name in TASKS:
            ...
            for dn in data.keys():
                ...
                fz = wrench[:, 2:3]
                action = np.concatenate([acts[:, :6], fz, acts[:, 6:7]], axis=1).astype(np.float32)
                n = len(acts)
                steps = []
                for t in range(n):
                    steps.append({
